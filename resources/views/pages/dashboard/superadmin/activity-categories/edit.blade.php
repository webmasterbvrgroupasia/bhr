...
@section('page-content')
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Add New Activity Category</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <section class="max-w-7xl">
        <form enctype="multipart/form-data" method="POST" enctype="multipart/form-data"
            action="{{ route('activity-categories.update',$category->id) }}"class="max-w-6xl grid grid-cols-12 gap-x-5 mt-4 px-5 md:px-8 lg:px-[100px] gap-y-5">
            @method('put')
            @csrf
            <div class="col-span-8">
                <label for="name" class="font-semibold">Category Name</label>
                <input type="text" name="name" id="name" required placeholder="Experience"
                    class="block w-full py-2 px-2 mt-3" value="{{$category->name}}">
            </div>
            <div class="col-span-8">
                <label for="keywords" class="font-semifold">Keywords Kategory</label>
                <input type="text" name="keywords" id="keywords" value="{{ $category->keywords}}" required placeholder="Keyword" class="block w-full py-2 px-2 mt-3">
            </div>
            <div class="col-span-8">
                <label for="images" class="font-semifold">Images</label>
                <input type="file" name="images" id="images" class="block w-full border border-black mt-3 " >
            </div>
            <div class="col-span-8">
                <button type="submit"
