    ...
    <div class="grid grid-cols-12 gap-6 items-start" itemscope itemtype="https://www.bvrbaliholidayrentals.com/activities">
        ...
        <div class="col-span-12 md:col-span-12 lg:col-span-8 space-y-[32px] order-first">
            @foreach ($categories as $category)
                    <a href="/activities/category/{{ $category->id }}" title="{{ $category->name}}"
                        class="col-span-2 flex items-center md:col-span-1 p-5 lg:p-12 bg-center bg-cover"
                        style="background:linear-gradient(rgba(0,0,0,0.5),rgba(0,0,0,0.5)),url('{{ asset('storage/'.$category->images)}}');
                        background-size: cover;
                        background-position:center;">
