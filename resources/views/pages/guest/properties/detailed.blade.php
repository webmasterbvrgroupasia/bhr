    ...
    <main class="space-y-[24px] w-full mx-auto mt-2 md:mt-4 lg:mt-8" itemscope>
        ...
        <div class="grid grid-cols-12 gap-x-7 items-start">
            ...
            ">
                    ...
                    <a href="{{ $property->booking_link }}" title="Book Now"
        ...
        @if ($property->link_youtube)
            <div class="col-span-2">
                ...
                <iframe class="w-2/3 h-40 md:h-64 lg:h-80 mt-5"
                    src="https://www.youtube.com/embed/{{ $property->link_youtube }}" title="YouTube video player"
                    frameborder="0"
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            @if ($property->pool_images)
                ...
                @foreach ($pool_images as $pool_image)
                    <div>
                        <button @click="openImageLightbox1 =! openImageLightbox1">
                            <img class="h-52 w-96 object-cover rounded-lg" src="{{ asset('storage/' . $pool_image) }}"
                                alt="{{ $property->name }} Pool" title="{{ $property->name }} Pool">
                        </button>
                        </button>
                    </div>
                @endforeach
                    ...
                    <div>
                        <button @click="openImageLightbox1 =! openImageLightbox1">
                            <img class="h-52 w-96 object-cover rounded-lg"
                                src="{{ asset('storage/' . $restaurant_image) }}" alt="{{ $property->name }} Restaurant"
                                title="{{ $property->name }} Restaurant">
                        </button>
                        </button>
                    </div>
                @endforeach
            ...
            @if ($property->room_images)
                ...
                @foreach ($room_images as $room_image)
                    <div>
                        <button @click="openImageLightbox1 =! openImageLightbox1">
                            <img class="h-52 w-96 object-cover rounded-lg" src="{{ asset('storage/' . $room_image) }}"
                                alt="{{ $property->name }} Room(s)" title="{{ $property->name }} Room(s)">
                        </button>
                        </button>
                    </div>
                @endforeach
        ...
        <section class="py-16 md:py-24 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach ($similiarProperties as $similiarProperty)
                    @php
                        $singleImage = explode("," , $similiarProperty->images);
                        ...
                        <div class="font-semibold p-4">
                            ...
                            <div class="font-normal text-gray-600">
                                <a class="text-blue-800"
                                    href="/areas/{{ Str::lower($similiarProperty->location->location) }}">{{ $similiarProperty->location->location }}</a>,
                                Indonesia
                            </div>
                            <a href="/properties/{{ $similiarProperty->slug }}"
